...
Cell::Cell(size_t r, size_t c)
    : Subject<Info, State>{}, Observer<Info, State>{}, r{r}, c{c},
...
void Cell::notify(Subject<Info, State> &whoFrom) {
  ...
  if (senderSignal.type == StateType::NewPiece) {
    ...
    State myNewState =
        State{StateType::Relay, senderSignal.colour, fromDirection};
    setState(myNewState);

  } else if (senderSignal.type == StateType::Relay) {
    if (fromDirection != senderSignal.direction)
      return; // notif is not intended for us
...
Info Cell::getInfo() const { return Info{r, c, colour}; }
