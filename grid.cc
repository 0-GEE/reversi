...
bool validMove(const vector<vector<Cell>> &grid, const size_t r, const size_t c) {
    // check for out of bounds
    size_t max = grid.size();
    if ((r < 0) || (c < 0) || (r >= max) || (c >= max)) return false;

    // check if cell already occupied
    Cell cellCandidate = grid[r][c];
    if (cellCandidate.getInfo().colour != Colour::NoColour) return false;
    return true;
}
...
bool Grid::isFull() const {
    for (auto row : theGrid) {
        for (auto cell : row) {
            if (cell.getInfo().colour == Colour::NoColour) return false;
        }
    }
    return true;
}
...
Colour Grid::whoWon() const {
    if (!isFull()) return Colour::NoColour;
    ...
    if (whiteCount > blackCount) return Colour::White;
...
bool Grid::setPiece(size_t r, size_t c, Colour colour) {
    if (!validMove(theGrid, r, c)) return false;
